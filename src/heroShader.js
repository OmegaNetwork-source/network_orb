export const heroVertexShader = `
    ...
    uniform vec3 u_hotContinentPos;
    uniform float u_hotBeamIntensity;
    ...
    void main() {
        ...
        gl_Position = projectionMatrix * viewPosition;
            ...
            float distToHot = length(normalize(a_instancePos) - normalize(u_hotContinentPos));
            v_hotFactor = smoothstep(0.4, 0.0, distToHot) * u_hotBeamIntensity;
...
export const heroFragmentShader = `
    ...
    void main() {
        ...
        float NdV = max(0., dot(viewNormal, V));
        ...
        vec3 tint = mix(baseColor, v_color * 0.3, smoothstep(0.15, 0.4, colorLuma));
        ...
        // Wet metallic specular highlight
        vec3 H = normalize(V + L);
        float spec = pow(max(0.0, dot(H, viewNormal)), 120.0);
        color += vec3(0.35, 0.37, 0.4) * spec * shadow;

        // Fresnel rim - venom wet edge glow
        float fresnel = pow(1.0 - NdV, 4.0);
        color += vec3(0.08, 0.08, 0.12) * fresnel;

        // Subtle hot beam (much more restrained)
        color += u_hotColor * v_hotFactor * 0.15 * (1.0 + 0.3 * sin(u_noiseCoordOffset.x * 10.0));
